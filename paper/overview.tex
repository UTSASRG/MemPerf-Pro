...
Based on our understanding, the memory overhead comes from three aspects: the metadata overhead, the alignment (or internal fragmentation), and the memory blowup. The metadata overhead indicates the physical space used for tracking objects or others. As described in Section~\ref{sec:allocator}, the alignment overhead is introduced due to the difference between the requested size and size class (or mapped pages). The memory blowup can be caused due to two reasons. First, a concurrent allocator does not satisfy the request immediately with freed objects. Second, some allocators, especially secure allocators, typically skip some objects. For the memory overhead, \MP{} aims to determine the ratio of each portion, so that it could guide allocator developers to further reduce the memory overhead based on the profiling results. 
