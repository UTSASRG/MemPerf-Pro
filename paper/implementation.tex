...
\MP{} predicts the performance impact that can be caused by the allocator by replacing the current allocator with a well-performed allocator. Currently, we are using the data of . 


\todo{How to deal with madvise and munmap?} 

\todo{\MP{} will predict whether the performance can be improved or not by changing the cycles of memory operations. If not, then the performance difference of an allocator will be caused by some application-friendliness. }
...
\MP{} will automatically detect the critical path for multithreaded applications. For each parallel phase, \MP{} select the thread with most cycles as its critical thread, and calculate the performance improvement of the critical path as the improvement of the whole program. Besides, \MP{} will gather the number of pages faults outside allocations for all threads, in order to indicate implict factors triggered by allocators that could influence the performance.
...
\MP{} will seperately determine allocation types in the allocator chosen for prediction, in which way it could handle the allocation type difference.
