...
%\MP{} also reports 52\% memory waste caused by memory blowup for this application. That helps explain why glibc-2.21 are using 740 MB more memory than TcMalloc.

%\paragraph{jemalloc:} jemalloc typically has good performance, but has a greater memory consumption caused by both memory blowup and external fragmentation. For \texttt{freqmine} application, jemalloc has 43\% memory waste caused by blowup. That helps explain that why it consumes 40\% more memory than TcMalloc, and 38\% more than glibc-2.21. In comparison, TcMalloc only has 2\% memory waste and glibc-2.21 has 7\% memory waste.
\paragraph{Hoard:} 
...
%Hoard also has 40\% memory waste for \texttt{swaptions}, where 26\% is from its external fragmentation.
