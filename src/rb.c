            ...
            struct libavl_allocator *allocator)
{
  ...
  tree = (rb_table *)allocator->libavl_malloc (allocator, sizeof *tree);
  if (tree == NULL)
    return NULL;
  ...
  for (p = tree->rb_root; p != NULL; )
    {
      ...
      if (cmp < 0) {
				//fprintf(stderr, "%s %d : node in tree was larger than item = %p (prev = %p)\n", __FUNCTION__, __LINE__, p, prev);
        p = p->rb_link[0];
      } else if (cmp > 0) {
        prev = p;
        p = p->rb_link[1];
...
rb_probe (struct rb_table *tree, void *item)
{
  ...
  n = pa[k - 1]->rb_link[da[k - 1]] = (rb_node *)
...
/* Replaces the current item in |trav| by |new_node| and returns the item replaced.
   |trav| must not have the null item selected.
   The new item must not upset the ordering of the tree. */
void *
rb_t_replace (struct rb_traverser *trav, void *new_node)
{
  void *old;

  assert (trav != NULL && trav->rb_node != NULL && new_node != NULL);
  old = trav->rb_node->rb_data;
  trav->rb_node->rb_data = new_node;
  return old;
}

/* Destroys |newt| with |rb_destroy (newt, destroy)|,
   first setting right links of nodes in |stack| within |newt|
   to null pointers to avoid touching uninitialized data. */
static void
copy_error_recovery (struct rb_node **stack, int height,
                     struct rb_table *newt, rb_item_func *destroy)
{
  assert (stack != NULL && height >= 0 && newt != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->rb_link[1] = NULL;
  rb_destroy (newt, destroy);
}
          ...
          rb_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct rb_table *newt;
  const struct rb_node *x;
  struct rb_node *y;

  assert (org != NULL);
  newt = rb_create (org->rb_compare, org->rb_param,
                    allocator != NULL ? allocator : org->rb_alloc);
  if (newt == NULL)
    return NULL;
  newt->rb_count = org->rb_count;
  if (newt->rb_count == 0)
    return newt;

  x = (const struct rb_node *) &org->rb_root;
  y = (struct rb_node *) &newt->rb_root;
  for (;;)
    {
      while (x->rb_link[0] != NULL)
        {
          ...
          y->rb_link[0] =
            (rb_node *)newt->rb_alloc->libavl_malloc (newt->rb_alloc,
                                           sizeof *y->rb_link[0]);
          if (y->rb_link[0] == NULL)
            {
              if (y != (struct rb_node *) &newt->rb_root)
                {
                  y->rb_data = NULL;
                  y->rb_link[1] = NULL;
                }

              copy_error_recovery (stack, height, newt, destroy);
              return NULL;
            }
