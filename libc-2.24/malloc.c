...
#define __USE_MISC
#include <stdint.h>   /* for __libc_enable_secure */
...
do_check_chunk (mstate av, mchunkptr p)
{
  ...
  if (!chunk_is_mmapped (p))
  ...
  else if (!DUMPED_MAIN_ARENA_CHUNK (p))
    {
      ...
      assert (((p->prev_size + sz) & (getpagesize() - 1)) == 0);
      /* mem is aligned */
...
munmap_chunk (mchunkptr p)
{
  ...
  /* Unfortunately we have to do the compilers job by hand here.  Normally
     ...
     the bit test.  */
  if (__builtin_expect (((block | total_size) & (getpagesize() - 1)) != 0, 0))
    {
...
mremap_chunk (mchunkptr p, size_t new_size)
{
  ...
  assert (chunk_is_mmapped (p));
  assert (((size + offset) & (getpagesize() - 1)) == 0);
...
malloc_printerr (int action, const char *str, void *ptr, mstate ar_ptr)
{
  ...
  if ((action & 5) == 5) {
  ...
  else if (action & 1)
    {
      ...
      fprintf (stderr, "*** Error in `%s': %s: 0x%s ***\n",
